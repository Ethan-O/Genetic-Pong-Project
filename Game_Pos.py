...
def ball_init(right):
    ...
    while abs(vert) < 3:
        ...
        vert= round(9*np.sin(rand_num))
        horz= round(9*np.cos(rand_num))        
...
class AI:
    ...
    def respond(self,ball_pos,pos):
        ...
        inputs = np.array([ball_px,ball_py,pos])
...
f= plt.figure()
fit_graph = f.add_subplot(211)
#fit_graph.plot([1,2],[1,2])
#f.show()
...
def fit_population(ais, show):
    ...
    last_mean_score  = np.mean(np.array(fit_scores))
    fit_scores_hist.append(last_mean_score)
    
    fit_graph.plot(list(range(len(fit_scores_hist))), fit_scores_hist)
    f.show()
